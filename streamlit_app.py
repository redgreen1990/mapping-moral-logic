...
def match_category(text: str):
    """
    Prefer specific topic matches (e.g., 'trump') over generic tags (e.g., 'racist').
    Then try sample_claim strings, then tags. Finally, try general principles by name.
    Returns ('entry'|'principle', dict) or None.
    """
    ...
    principles = LIBRARY.get("general_principles", []) or []

    # 1) Entries by TOPIC first (most specific)
    for entry in entries:
        topic = entry.get("topic", "")
        if isinstance(topic, str) and topic and topic.lower() in t:
            return ("entry", entry)

    # 2) Entries by sample_claim (medium-specific)
    for entry in entries:
        sc = entry.get("sample_claim", [])
        if isinstance(sc, str):
            sc = [sc]
        for s in sc or []:
            if isinstance(s, str) and s and s.lower() in t:
                return ("entry", entry)

    # 3) Entries by tags (most generic)
    for entry in entries:
        for tag in entry.get("tags", []) or []:
            if isinstance(tag, str) and tag and tag.lower() in t:
                return ("entry", entry)

    # 4) General principles by name
    for gp in principles:
        name = gp.get("name", "")
        if isinstance(name, str) and name and name.lower() in t:
            return ("principle", gp)
